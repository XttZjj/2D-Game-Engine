...
public:
	...
	static inline std::unordered_map<std::string, SDL_GameControllerButton> keycode_to_buttoncode = {
	...
	static bool GetControllerButton(std::string keycode);
	static bool GetControllerButtonDown(std::string keycode);
	static bool GetControllerButtonUp(std::string keycode);
	...
	static int GetLeftXAxis();

	static int GetLeftYAxis();

	static int GetRightXAxis();

	static int GetRightYAxis();

	static void KeyPress(std::string key);

	static void KeyPressEnd(std::string key);
...
private:
	...
	static inline std::vector<int> just_become_down_buttons;
	static inline std::vector<int> just_become_up_buttons;

	static inline float mouse_scroll_this_frame = 0;

	static inline std::unordered_map<SDL_GameControllerButton, INPUT_STATE> gamebutton_state;
	static inline std::vector<SDL_GameControllerButton> just_became_down_gamebutton;
	static inline std::vector<SDL_GameControllerButton> just_became_up_gamebutton;

	static inline int left_x_axis;
	static inline int left_y_axis;
	static inline int right_x_axis;
	static inline int right_y_axis;

	static inline SDL_Scancode justPressedKey;
};
